...
def train_net(net_h, net_pl, net_s, device, data_dir, epochs=140,
    ...
    dir_checkpoint = 'checkpoints_model/DST_128/Hist_PDFLab_sRGB/16d/'  # check points directory
  ...
  ''')
    ...
    for epoch in range(epochs):
        ...
        if ((epoch + 1) % chkpoint_period == 0) or ((epoch + 1) >200):
...
def get_args():
  ...
  """
    ...
    parser.add_argument('-b', '--batch-size', metavar='B', type=int, nargs='?', default=50, help='Batch size', dest='batch_size')
    ...
    parser.add_argument('-wbs', '--wb-settings', dest='wb_settings', nargs='+', default=['D', 'S', 'T']) 
...
if __name__ == '__main__':
    ...
    hist_net = ABCFormerM.HistNet(device=device, inchnls=3 * len(args.wb_settings), em_dim=args.embed_dim)
    PDFlab_net = ABCFormerM.PDFLabNet(device=device, inchnls=3 * len(args.wb_settings), em_dim=args.embed_dim)
    sRGB_net = ABCFormerM.sRGBNet(device=device, inchnls=3 * len(args.wb_settings), em_dim=args.embed_dim)
